#ifndef WIRELINK_GENERATED_FCI_DEVICE_RUNTIME_H
#define WIRELINK_GENERATED_FCI_DEVICE_RUNTIME_H
...
#define FCI_DEVICE_SCHEMA_IDENTITY UINT64_C(0xC4D48A693E596565)
#define FCI_DEVICE_BINDING_PROFILE_IDENTITY UINT64_C(0x86107756B707FC65)
#define FCI_DEVICE_BINDING_PROFILE_VERSION 1U
#define FCI_DEVICE_IDENTITY_ALGORITHM "fnv1a64-v1"

#define FCI_DEVICE_RUNTIME_CODEGEN_ABI_VERSION 32U

/* Generated capabilities, not application overrides. */
#define FCI_DEVICE_RUNTIME_HAS_RPC_CLIENT 1
#define FCI_DEVICE_RUNTIME_HAS_RPC_SERVER 0

#define FCI_DEVICE_RPC_REQUEST_FINGERPRINT_ALGORITHM "fnv1a64-canonical-request-v1"

typedef int32_t fci_device_runtime_domain_t;
enum {
  FCI_DEVICE_RUNTIME_OK = 0,
  FCI_DEVICE_RUNTIME_NON_RX,
  FCI_DEVICE_RUNTIME_UNKNOWN_MESSAGE,
  FCI_DEVICE_RUNTIME_MISSING_ROUTE,
  FCI_DEVICE_RUNTIME_MISSING_SCRATCH,
  FCI_DEVICE_RUNTIME_DELIVERY_MISMATCH,
  FCI_DEVICE_RUNTIME_CODEC_ERROR,
  FCI_DEVICE_RUNTIME_STORAGE_ERROR,
  FCI_DEVICE_RUNTIME_RPC_ERROR,
  FCI_DEVICE_RUNTIME_CORE_ERROR,
  FCI_DEVICE_RUNTIME_APPLICATION_ERROR,
  FCI_DEVICE_RUNTIME_INVALID_ARGUMENT
};

typedef uint8_t fci_device_runtime_detail_kind_t;
enum {
  FCI_DEVICE_RUNTIME_DETAIL_NONE = 0,
  FCI_DEVICE_RUNTIME_DETAIL_RETAINED = 1,
  FCI_DEVICE_RUNTIME_DETAIL_RPC = 2
};
...
} fci_device_runtime_retained_detail_t;
...
} fci_device_runtime_rpc_detail_t;

#define FCI_DEVICE_RUNTIME_DETAIL_DIRECT 3U
typedef struct {
  wl_codec_status_t codec_status;
  int32_t application_result;
} fci_device_runtime_direct_detail_t;

typedef union {
  fci_device_runtime_direct_detail_t direct;
  fci_device_runtime_retained_detail_t retained;
  fci_device_runtime_rpc_detail_t rpc;
} fci_device_runtime_detail_t;
...
typedef struct {
  fci_device_runtime_domain_t domain;
  wl_event_type_t event_type;
  uint16_t message_id;
  fci_device_runtime_detail_kind_t detail_kind;
  uint8_t event_consumed;
  fci_device_runtime_detail_t detail;
} fci_device_runtime_result_t;
...
static inline bool fci_device_runtime_result_ok(const fci_device_runtime_result_t *result) {
  return result != NULL && result->domain == FCI_DEVICE_RUNTIME_OK;
}

const char *fci_device_runtime_result_str(const fci_device_runtime_result_t *result);

static inline const fci_device_runtime_retained_detail_t *fci_device_runtime_result_retained_detail(const fci_device_runtime_result_t *result) {
  return result != NULL && result->detail_kind == FCI_DEVICE_RUNTIME_DETAIL_RETAINED ? &result->detail.retained : NULL;
}

static inline const fci_device_runtime_rpc_detail_t *fci_device_runtime_result_rpc_detail(const fci_device_runtime_result_t *result) {
  return result != NULL && result->detail_kind == FCI_DEVICE_RUNTIME_DETAIL_RPC ? &result->detail.rpc : NULL;
}

#define FCI_DEVICE_RUNTIME_HAS_MANAGED_RPC 1
/* The typed value remains borrowed until the matching release. */
typedef struct {
  const arm_status_t *value;
  uint32_t generation;
  wl_latest_view_t lease;
} fci_device_arm_status_latest_view_t;
...
typedef struct {
  ...
  uint32_t generation;
  wl_latest_view_t lease;
} fci_device_motor_feedback_latest_view_t;
...
typedef struct {
  ...
  uint32_t generation;
  wl_latest_view_t lease;
} fci_device_arm_diagnostics_latest_view_t;

/* Borrowed input and nested bytes/strings expire when the callback returns.
...
typedef int32_t (*fci_device_bulk_status_direct_fn)(void *user_data, const bulk_status_t *message, wl_delivery_t delivery);
typedef struct {
  fci_device_bulk_status_direct_fn handler;
  void *user_data;
  bulk_status_t *scratch;
} fci_device_bulk_status_direct_t;
...
} fci_device_acquire_control_lease_request_token_t;
...
typedef int32_t (*fci_device_acquire_control_lease_rpc_request_handler_fn)(void *user_data,
    const acquire_control_lease_request_t *request, const fci_device_acquire_control_lease_request_token_t *token,
...
typedef int32_t (*fci_device_acquire_control_lease_handler_fn)(void *user_data,
...
typedef void (*fci_device_acquire_control_lease_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_acquire_control_lease_rpc_request_handler_fn request_handler;
  void *user_data;
#if ACQUIRE_CONTROL_LEASE_REQUEST_HAS_VALUE && ACQUIRE_CONTROL_LEASE_RESPONSE_HAS_VALUE
  fci_device_acquire_control_lease_handler_fn value_handler;
...
#endif
} fci_device_acquire_control_lease_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_clear_error_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_clear_error_rpc_request_handler_fn)(void *user_data,
    const clear_error_request_t *request, const fci_device_clear_error_request_token_t *token,
    wl_delivery_t delivery);

#if CLEAR_ERROR_REQUEST_HAS_VALUE && CLEAR_ERROR_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_clear_error_handler_fn)(void *user_data,
    const clear_error_request_value_t *request, clear_error_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_clear_error_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_clear_error_rpc_request_handler_fn request_handler;
  void *user_data;
#if CLEAR_ERROR_REQUEST_HAS_VALUE && CLEAR_ERROR_RESPONSE_HAS_VALUE
  fci_device_clear_error_handler_fn value_handler;
  void *value_user_data;
  clear_error_request_value_t *request_value;
  clear_error_response_value_t *response_value;
#endif
} fci_device_clear_error_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_clear_faults_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_clear_faults_rpc_request_handler_fn)(void *user_data,
    const clear_faults_request_t *request, const fci_device_clear_faults_request_token_t *token,
    wl_delivery_t delivery);

#if CLEAR_FAULTS_REQUEST_HAS_VALUE && CLEAR_FAULTS_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_clear_faults_handler_fn)(void *user_data,
    const clear_faults_request_value_t *request, clear_faults_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_clear_faults_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_clear_faults_rpc_request_handler_fn request_handler;
  void *user_data;
#if CLEAR_FAULTS_REQUEST_HAS_VALUE && CLEAR_FAULTS_RESPONSE_HAS_VALUE
  fci_device_clear_faults_handler_fn value_handler;
  void *value_user_data;
  clear_faults_request_value_t *request_value;
  clear_faults_response_value_t *response_value;
#endif
} fci_device_clear_faults_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_emergency_stop_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_emergency_stop_rpc_request_handler_fn)(void *user_data,
    const emergency_stop_request_t *request, const fci_device_emergency_stop_request_token_t *token,
    wl_delivery_t delivery);

#if EMERGENCY_STOP_REQUEST_HAS_VALUE && EMERGENCY_STOP_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_emergency_stop_handler_fn)(void *user_data,
    const emergency_stop_request_value_t *request, emergency_stop_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_emergency_stop_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_emergency_stop_rpc_request_handler_fn request_handler;
  void *user_data;
#if EMERGENCY_STOP_REQUEST_HAS_VALUE && EMERGENCY_STOP_RESPONSE_HAS_VALUE
  fci_device_emergency_stop_handler_fn value_handler;
  void *value_user_data;
  emergency_stop_request_value_t *request_value;
  emergency_stop_response_value_t *response_value;
#endif
} fci_device_emergency_stop_rpc_t;
...
typedef int32_t (*fci_device_get_boot_status_rpc_request_handler_fn)(void *user_data, const get_boot_status_request_t *request, const wl_rpc_server_request_t *server_request, wl_delivery_t delivery);
typedef struct {
  get_boot_status_request_t *request_scratch;
  get_boot_status_response_t *response_scratch;
  fci_device_get_boot_status_rpc_request_handler_fn request_handler;
  void *user_data;
} fci_device_get_boot_status_rpc_t;
...
} fci_device_get_device_info_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_get_device_info_rpc_request_handler_fn)(void *user_data,
    const get_device_info_request_t *request, const fci_device_get_device_info_request_token_t *token,
    wl_delivery_t delivery);

#if GET_DEVICE_INFO_REQUEST_HAS_VALUE && GET_DEVICE_INFO_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_get_device_info_handler_fn)(void *user_data,
    const get_device_info_request_value_t *request, get_device_info_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_get_device_info_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_get_device_info_rpc_request_handler_fn request_handler;
  void *user_data;
#if GET_DEVICE_INFO_REQUEST_HAS_VALUE && GET_DEVICE_INFO_RESPONSE_HAS_VALUE
  fci_device_get_device_info_handler_fn value_handler;
  void *value_user_data;
  get_device_info_request_value_t *request_value;
  get_device_info_response_value_t *response_value;
#endif
} fci_device_get_device_info_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_get_device_settings_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_get_device_settings_rpc_request_handler_fn)(void *user_data,
    const get_device_settings_request_t *request, const fci_device_get_device_settings_request_token_t *token,
    wl_delivery_t delivery);

#if GET_DEVICE_SETTINGS_REQUEST_HAS_VALUE && GET_DEVICE_SETTINGS_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_get_device_settings_handler_fn)(void *user_data,
    const get_device_settings_request_value_t *request, get_device_settings_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_get_device_settings_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_get_device_settings_rpc_request_handler_fn request_handler;
  void *user_data;
#if GET_DEVICE_SETTINGS_REQUEST_HAS_VALUE && GET_DEVICE_SETTINGS_RESPONSE_HAS_VALUE
  fci_device_get_device_settings_handler_fn value_handler;
  void *value_user_data;
  get_device_settings_request_value_t *request_value;
  get_device_settings_response_value_t *response_value;
#endif
} fci_device_get_device_settings_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_get_motor_feedback_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_get_motor_feedback_rpc_request_handler_fn)(void *user_data,
    const get_motor_feedback_request_t *request, const fci_device_get_motor_feedback_request_token_t *token,
    wl_delivery_t delivery);

#if GET_MOTOR_FEEDBACK_REQUEST_HAS_VALUE && GET_MOTOR_FEEDBACK_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_get_motor_feedback_handler_fn)(void *user_data,
    const get_motor_feedback_request_value_t *request, get_motor_feedback_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_get_motor_feedback_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_get_motor_feedback_rpc_request_handler_fn request_handler;
  void *user_data;
#if GET_MOTOR_FEEDBACK_REQUEST_HAS_VALUE && GET_MOTOR_FEEDBACK_RESPONSE_HAS_VALUE
  fci_device_get_motor_feedback_handler_fn value_handler;
  void *value_user_data;
  get_motor_feedback_request_value_t *request_value;
  get_motor_feedback_response_value_t *response_value;
#endif
} fci_device_get_motor_feedback_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_home_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_home_rpc_request_handler_fn)(void *user_data,
    const home_request_t *request, const fci_device_home_request_token_t *token,
    wl_delivery_t delivery);

#if HOME_REQUEST_HAS_VALUE && HOME_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_home_handler_fn)(void *user_data,
    const home_request_value_t *request, home_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_home_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_home_rpc_request_handler_fn request_handler;
  void *user_data;
#if HOME_REQUEST_HAS_VALUE && HOME_RESPONSE_HAS_VALUE
  fci_device_home_handler_fn value_handler;
  void *value_user_data;
  home_request_value_t *request_value;
  home_response_value_t *response_value;
#endif
} fci_device_home_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_motor_register_read_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_motor_register_read_rpc_request_handler_fn)(void *user_data,
    const motor_register_read_request_t *request, const fci_device_motor_register_read_request_token_t *token,
    wl_delivery_t delivery);

#if MOTOR_REGISTER_READ_REQUEST_HAS_VALUE && MOTOR_REGISTER_READ_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_motor_register_read_handler_fn)(void *user_data,
    const motor_register_read_request_value_t *request, motor_register_read_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_motor_register_read_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_motor_register_read_rpc_request_handler_fn request_handler;
  void *user_data;
#if MOTOR_REGISTER_READ_REQUEST_HAS_VALUE && MOTOR_REGISTER_READ_RESPONSE_HAS_VALUE
  fci_device_motor_register_read_handler_fn value_handler;
  void *value_user_data;
  motor_register_read_request_value_t *request_value;
  motor_register_read_response_value_t *response_value;
#endif
} fci_device_motor_register_read_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_motor_register_write_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_motor_register_write_rpc_request_handler_fn)(void *user_data,
    const motor_register_write_request_t *request, const fci_device_motor_register_write_request_token_t *token,
    wl_delivery_t delivery);

#if MOTOR_REGISTER_WRITE_REQUEST_HAS_VALUE && MOTOR_REGISTER_WRITE_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_motor_register_write_handler_fn)(void *user_data,
    const motor_register_write_request_value_t *request, motor_register_write_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_motor_register_write_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_motor_register_write_rpc_request_handler_fn request_handler;
  void *user_data;
#if MOTOR_REGISTER_WRITE_REQUEST_HAS_VALUE && MOTOR_REGISTER_WRITE_RESPONSE_HAS_VALUE
  fci_device_motor_register_write_handler_fn value_handler;
  void *value_user_data;
  motor_register_write_request_value_t *request_value;
  motor_register_write_response_value_t *response_value;
#endif
} fci_device_motor_register_write_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_motor_set_zero_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_motor_set_zero_rpc_request_handler_fn)(void *user_data,
    const motor_set_zero_request_t *request, const fci_device_motor_set_zero_request_token_t *token,
    wl_delivery_t delivery);

#if MOTOR_SET_ZERO_REQUEST_HAS_VALUE && MOTOR_SET_ZERO_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_motor_set_zero_handler_fn)(void *user_data,
    const motor_set_zero_request_value_t *request, motor_set_zero_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_motor_set_zero_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_motor_set_zero_rpc_request_handler_fn request_handler;
  void *user_data;
#if MOTOR_SET_ZERO_REQUEST_HAS_VALUE && MOTOR_SET_ZERO_RESPONSE_HAS_VALUE
  fci_device_motor_set_zero_handler_fn value_handler;
  void *value_user_data;
  motor_set_zero_request_value_t *request_value;
  motor_set_zero_response_value_t *response_value;
#endif
} fci_device_motor_set_zero_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_motor_store_parameters_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_motor_store_parameters_rpc_request_handler_fn)(void *user_data,
    const motor_store_parameters_request_t *request, const fci_device_motor_store_parameters_request_token_t *token,
    wl_delivery_t delivery);

#if MOTOR_STORE_PARAMETERS_REQUEST_HAS_VALUE && MOTOR_STORE_PARAMETERS_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_motor_store_parameters_handler_fn)(void *user_data,
    const motor_store_parameters_request_value_t *request, motor_store_parameters_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_motor_store_parameters_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_motor_store_parameters_rpc_request_handler_fn request_handler;
  void *user_data;
#if MOTOR_STORE_PARAMETERS_REQUEST_HAS_VALUE && MOTOR_STORE_PARAMETERS_RESPONSE_HAS_VALUE
  fci_device_motor_store_parameters_handler_fn value_handler;
  void *value_user_data;
  motor_store_parameters_request_value_t *request_value;
  motor_store_parameters_response_value_t *response_value;
#endif
} fci_device_motor_store_parameters_rpc_t;
/* The decoded request and its borrowed fields are valid only for the
 * callback. Copy server_request for asynchronous completion. Its generation
 * prevents a late completion from targeting a reused request identity. A
 * nonzero return abandons this exact pending operation. */
typedef int32_t (*fci_device_reboot_rpc_request_handler_fn)(void *user_data, const reboot_request_t *request, const wl_rpc_server_request_t *server_request, wl_delivery_t delivery);
typedef struct {
  reboot_request_t *request_scratch;
  reboot_response_t *response_scratch;
  fci_device_reboot_rpc_request_handler_fn request_handler;
  void *user_data;
} fci_device_reboot_rpc_t;
...
} fci_device_release_control_lease_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_release_control_lease_rpc_request_handler_fn)(void *user_data,
    const release_control_lease_request_t *request, const fci_device_release_control_lease_request_token_t *token,
    wl_delivery_t delivery);

#if RELEASE_CONTROL_LEASE_REQUEST_HAS_VALUE && RELEASE_CONTROL_LEASE_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_release_control_lease_handler_fn)(void *user_data,
    const release_control_lease_request_value_t *request, release_control_lease_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_release_control_lease_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_release_control_lease_rpc_request_handler_fn request_handler;
  void *user_data;
#if RELEASE_CONTROL_LEASE_REQUEST_HAS_VALUE && RELEASE_CONTROL_LEASE_RESPONSE_HAS_VALUE
  fci_device_release_control_lease_handler_fn value_handler;
  void *value_user_data;
  release_control_lease_request_value_t *request_value;
  release_control_lease_response_value_t *response_value;
#endif
} fci_device_release_control_lease_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_set_arm_control_mode_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_set_arm_control_mode_rpc_request_handler_fn)(void *user_data,
    const set_arm_control_mode_request_t *request, const fci_device_set_arm_control_mode_request_token_t *token,
    wl_delivery_t delivery);

#if SET_ARM_CONTROL_MODE_REQUEST_HAS_VALUE && SET_ARM_CONTROL_MODE_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_set_arm_control_mode_handler_fn)(void *user_data,
    const set_arm_control_mode_request_value_t *request, set_arm_control_mode_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_set_arm_control_mode_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_set_arm_control_mode_rpc_request_handler_fn request_handler;
  void *user_data;
#if SET_ARM_CONTROL_MODE_REQUEST_HAS_VALUE && SET_ARM_CONTROL_MODE_RESPONSE_HAS_VALUE
  fci_device_set_arm_control_mode_handler_fn value_handler;
  void *value_user_data;
  set_arm_control_mode_request_value_t *request_value;
  set_arm_control_mode_response_value_t *response_value;
#endif
} fci_device_set_arm_control_mode_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_set_arm_mode_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_set_arm_mode_rpc_request_handler_fn)(void *user_data,
    const set_arm_mode_request_t *request, const fci_device_set_arm_mode_request_token_t *token,
    wl_delivery_t delivery);

#if SET_ARM_MODE_REQUEST_HAS_VALUE && SET_ARM_MODE_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_set_arm_mode_handler_fn)(void *user_data,
    const set_arm_mode_request_value_t *request, set_arm_mode_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_set_arm_mode_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_set_arm_mode_rpc_request_handler_fn request_handler;
  void *user_data;
#if SET_ARM_MODE_REQUEST_HAS_VALUE && SET_ARM_MODE_RESPONSE_HAS_VALUE
  fci_device_set_arm_mode_handler_fn value_handler;
  void *value_user_data;
  set_arm_mode_request_value_t *request_value;
  set_arm_mode_response_value_t *response_value;
#endif
} fci_device_set_arm_mode_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_set_device_info_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_set_device_info_rpc_request_handler_fn)(void *user_data,
    const set_device_info_request_t *request, const fci_device_set_device_info_request_token_t *token,
    wl_delivery_t delivery);

#if SET_DEVICE_INFO_REQUEST_HAS_VALUE && SET_DEVICE_INFO_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_set_device_info_handler_fn)(void *user_data,
    const set_device_info_request_value_t *request, set_device_info_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_set_device_info_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_set_device_info_rpc_request_handler_fn request_handler;
  void *user_data;
#if SET_DEVICE_INFO_REQUEST_HAS_VALUE && SET_DEVICE_INFO_RESPONSE_HAS_VALUE
  fci_device_set_device_info_handler_fn value_handler;
  void *value_user_data;
  set_device_info_request_value_t *request_value;
  set_device_info_response_value_t *response_value;
#endif
} fci_device_set_device_info_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_set_device_settings_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_set_device_settings_rpc_request_handler_fn)(void *user_data,
    const set_device_settings_request_t *request, const fci_device_set_device_settings_request_token_t *token,
    wl_delivery_t delivery);

#if SET_DEVICE_SETTINGS_REQUEST_HAS_VALUE && SET_DEVICE_SETTINGS_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_set_device_settings_handler_fn)(void *user_data,
    const set_device_settings_request_value_t *request, set_device_settings_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_set_device_settings_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_set_device_settings_rpc_request_handler_fn request_handler;
  void *user_data;
#if SET_DEVICE_SETTINGS_REQUEST_HAS_VALUE && SET_DEVICE_SETTINGS_RESPONSE_HAS_VALUE
  fci_device_set_device_settings_handler_fn value_handler;
  void *value_user_data;
  set_device_settings_request_value_t *request_value;
  set_device_settings_response_value_t *response_value;
#endif
} fci_device_set_device_settings_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_set_gripper_control_mode_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_set_gripper_control_mode_rpc_request_handler_fn)(void *user_data,
    const set_gripper_control_mode_request_t *request, const fci_device_set_gripper_control_mode_request_token_t *token,
    wl_delivery_t delivery);

#if SET_GRIPPER_CONTROL_MODE_REQUEST_HAS_VALUE && SET_GRIPPER_CONTROL_MODE_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_set_gripper_control_mode_handler_fn)(void *user_data,
    const set_gripper_control_mode_request_value_t *request, set_gripper_control_mode_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_set_gripper_control_mode_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_set_gripper_control_mode_rpc_request_handler_fn request_handler;
  void *user_data;
#if SET_GRIPPER_CONTROL_MODE_REQUEST_HAS_VALUE && SET_GRIPPER_CONTROL_MODE_RESPONSE_HAS_VALUE
  fci_device_set_gripper_control_mode_handler_fn value_handler;
  void *value_user_data;
  set_gripper_control_mode_request_value_t *request_value;
  set_gripper_control_mode_response_value_t *response_value;
#endif
} fci_device_set_gripper_control_mode_rpc_t;
/* Copy the request token for deferred replies. Its private state is not
 * application data; it authorizes only this runtime and execution lifetime. */
typedef struct {
  struct {
...
} fci_device_set_zero_request_token_t;

/* Request fields are borrowed only for this callback. Nonzero return abandons
 * execution locally; send a rejection explicitly to notify the caller. */
typedef int32_t (*fci_device_set_zero_rpc_request_handler_fn)(void *user_data,
    const set_zero_request_t *request, const fci_device_set_zero_request_token_t *token,
    wl_delivery_t delivery);

#if SET_ZERO_REQUEST_HAS_VALUE && SET_ZERO_RESPONSE_HAS_VALUE
/* Ordinary immediate business handler: zero succeeds, nonzero rejects with
 * that application code. response is initialized before the callback. */
typedef int32_t (*fci_device_set_zero_handler_fn)(void *user_data,
    const set_zero_request_value_t *request, set_zero_response_value_t *response);
/* A response exists only on SUCCESS and lives through this callback. Assign
 * *response to save an independent value. No call-slot release is required. */
typedef void (*fci_device_set_zero_completion_fn)(void *user_data,
...
typedef struct {
  ...
  fci_device_set_zero_rpc_request_handler_fn request_handler;
  void *user_data;
#if SET_ZERO_REQUEST_HAS_VALUE && SET_ZERO_RESPONSE_HAS_VALUE
  fci_device_set_zero_handler_fn value_handler;
  void *value_user_data;
  set_zero_request_value_t *request_value;
  set_zero_response_value_t *response_value;
#endif
} fci_device_set_zero_rpc_t;
/* The decoded request and its borrowed fields are valid only for the
 * callback. Copy server_request for asynchronous completion. Its generation
 * prevents a late completion from targeting a reused request identity. A
 * nonzero return abandons this exact pending operation. */
typedef int32_t (*fci_device_start_upgrade_rpc_request_handler_fn)(void *user_data, const start_upgrade_request_t *request, const wl_rpc_server_request_t *server_request, wl_delivery_t delivery);
typedef struct {
  start_upgrade_request_t *request_scratch;
  start_upgrade_response_t *response_scratch;
  fci_device_start_upgrade_rpc_request_handler_fn request_handler;
  void *user_data;
} fci_device_start_upgrade_rpc_t;
...
typedef union {
  get_boot_status_request_t get_boot_status_request;
  get_boot_status_response_t get_boot_status_response;
  reboot_request_t reboot_request;
  reboot_response_t reboot_response;
  start_upgrade_request_t start_upgrade_request;
  start_upgrade_response_t start_upgrade_response;
} fci_device_runtime_rpc_encode_scratch_t;
...
} fci_device_runtime_poll_result_t;

typedef struct {
  fci_device_runtime_poll_result_t deadlines;
  fci_device_runtime_result_t response;
  uint16_t responses_submitted;
  uint16_t responses_deferred;
} fci_device_runtime_service_result_t;

typedef struct {
  uint8_t _reserved;
  fci_device_bulk_status_direct_t bulk_status_direct;
  ...
  fci_device_runtime_rpc_encode_scratch_t *rpc_encode_scratch;
  fci_device_acquire_control_lease_rpc_t acquire_control_lease;
  fci_device_clear_error_rpc_t clear_error;
  fci_device_clear_faults_rpc_t clear_faults;
  fci_device_emergency_stop_rpc_t emergency_stop;
  fci_device_get_boot_status_rpc_t get_boot_status;
  fci_device_get_device_info_rpc_t get_device_info;
  fci_device_get_device_settings_rpc_t get_device_settings;
  fci_device_get_motor_feedback_rpc_t get_motor_feedback;
  fci_device_home_rpc_t home;
  fci_device_motor_register_read_rpc_t motor_register_read;
  fci_device_motor_register_write_rpc_t motor_register_write;
  fci_device_motor_set_zero_rpc_t motor_set_zero;
  fci_device_motor_store_parameters_rpc_t motor_store_parameters;
  fci_device_reboot_rpc_t reboot;
  fci_device_release_control_lease_rpc_t release_control_lease;
  fci_device_set_arm_control_mode_rpc_t set_arm_control_mode;
  fci_device_set_arm_mode_rpc_t set_arm_mode;
  fci_device_set_device_info_rpc_t set_device_info;
  fci_device_set_device_settings_rpc_t set_device_settings;
  fci_device_set_gripper_control_mode_rpc_t set_gripper_control_mode;
  fci_device_set_zero_rpc_t set_zero;
  fci_device_start_upgrade_rpc_t start_upgrade;
} fci_device_runtime_t;

typedef void (*fci_device_runtime_result_fn)(void *user_data, const fci_device_runtime_result_t *result);

typedef struct {
  fci_device_runtime_t *runtime;
  void *user_data;
  fci_device_runtime_result_fn on_result;
  wl_rpc_err_t last_service_result;
  fci_device_runtime_service_result_t last_service;
} fci_device_runtime_pump_t;
...
typedef struct {
  uint8_t _reserved;
  fci_device_bulk_status_direct_fn bulk_status_direct_handler;
  void *bulk_status_direct_user_data;
  ...
  fci_device_acquire_control_lease_rpc_request_handler_fn acquire_control_lease_request_handler;
  void *acquire_control_lease_user_data;
  fci_device_clear_error_rpc_request_handler_fn clear_error_request_handler;
  void *clear_error_user_data;
  fci_device_clear_faults_rpc_request_handler_fn clear_faults_request_handler;
  void *clear_faults_user_data;
  fci_device_emergency_stop_rpc_request_handler_fn emergency_stop_request_handler;
  void *emergency_stop_user_data;
  fci_device_get_boot_status_rpc_request_handler_fn get_boot_status_request_handler;
  void *get_boot_status_user_data;
  fci_device_get_device_info_rpc_request_handler_fn get_device_info_request_handler;
  void *get_device_info_user_data;
  fci_device_get_device_settings_rpc_request_handler_fn get_device_settings_request_handler;
  void *get_device_settings_user_data;
  fci_device_get_motor_feedback_rpc_request_handler_fn get_motor_feedback_request_handler;
  void *get_motor_feedback_user_data;
  fci_device_home_rpc_request_handler_fn home_request_handler;
  void *home_user_data;
  fci_device_motor_register_read_rpc_request_handler_fn motor_register_read_request_handler;
  void *motor_register_read_user_data;
  fci_device_motor_register_write_rpc_request_handler_fn motor_register_write_request_handler;
  void *motor_register_write_user_data;
  fci_device_motor_set_zero_rpc_request_handler_fn motor_set_zero_request_handler;
  void *motor_set_zero_user_data;
  fci_device_motor_store_parameters_rpc_request_handler_fn motor_store_parameters_request_handler;
  void *motor_store_parameters_user_data;
  fci_device_reboot_rpc_request_handler_fn reboot_request_handler;
  void *reboot_user_data;
  fci_device_release_control_lease_rpc_request_handler_fn release_control_lease_request_handler;
  void *release_control_lease_user_data;
  fci_device_set_arm_control_mode_rpc_request_handler_fn set_arm_control_mode_request_handler;
  void *set_arm_control_mode_user_data;
  fci_device_set_arm_mode_rpc_request_handler_fn set_arm_mode_request_handler;
  void *set_arm_mode_user_data;
  fci_device_set_device_info_rpc_request_handler_fn set_device_info_request_handler;
  void *set_device_info_user_data;
  fci_device_set_device_settings_rpc_request_handler_fn set_device_settings_request_handler;
  void *set_device_settings_user_data;
  fci_device_set_gripper_control_mode_rpc_request_handler_fn set_gripper_control_mode_request_handler;
  void *set_gripper_control_mode_user_data;
  fci_device_set_zero_rpc_request_handler_fn set_zero_request_handler;
  void *set_zero_user_data;
  fci_device_start_upgrade_rpc_request_handler_fn start_upgrade_request_handler;
  void *start_upgrade_user_data;
} fci_device_runtime_config_t;

#define FCI_DEVICE_RUNTIME_HAS_DEFAULT_STORAGE 1
typedef union {
  uint8_t byte;
...
} fci_device_runtime_default_storage_alignment_t;

#if defined(__cplusplus)
#define FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_ALIGNMENT alignof(fci_device_runtime_default_storage_alignment_t)
#elif defined(_MSC_VER)
#define FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_ALIGNMENT __alignof(fci_device_runtime_default_storage_alignment_t)
#else
#define FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_ALIGNMENT _Alignof(fci_device_runtime_default_storage_alignment_t)
#endif
#define FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_CAPACITY \
  (1U + \
   ((FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_ALIGNMENT - 1U) + ((sizeof(arm_status_t) + (FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_ALIGNMENT - 1U)) * WL_LATEST_SLOT_COUNT)) + \
   ((FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_ALIGNMENT - 1U) + ((sizeof(motor_feedback_t) + (FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_ALIGNMENT - 1U)) * WL_LATEST_SLOT_COUNT)) + \
   ((FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_ALIGNMENT - 1U) + ((sizeof(arm_diagnostics_t) + (FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_ALIGNMENT - 1U)) * WL_LATEST_SLOT_COUNT)) + \
   ((FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_ALIGNMENT - 1U) + sizeof(wl_rpc_client_slot_t)) + \
   226U)

typedef union {
  fci_device_runtime_default_storage_alignment_t alignment;
  uint8_t bytes[FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_CAPACITY];
} fci_device_runtime_default_storage_t;

typedef union { acquire_control_lease_response_t response; } fci_device_runtime_acquire_control_lease_decode_detail_t;
typedef union { clear_error_response_t response; } fci_device_runtime_clear_error_decode_detail_t;
typedef union { clear_faults_response_t response; } fci_device_runtime_clear_faults_decode_detail_t;
typedef union { emergency_stop_response_t response; } fci_device_runtime_emergency_stop_decode_detail_t;
typedef union { get_boot_status_response_t response; } fci_device_runtime_get_boot_status_decode_detail_t;
typedef union { get_device_info_response_t response; } fci_device_runtime_get_device_info_decode_detail_t;
typedef union { get_device_settings_response_t response; } fci_device_runtime_get_device_settings_decode_detail_t;
typedef union { get_motor_feedback_response_t response; } fci_device_runtime_get_motor_feedback_decode_detail_t;
typedef union { home_response_t response; } fci_device_runtime_home_decode_detail_t;
typedef union { motor_register_read_response_t response; } fci_device_runtime_motor_register_read_decode_detail_t;
typedef union { motor_register_write_response_t response; } fci_device_runtime_motor_register_write_decode_detail_t;
typedef union { motor_set_zero_response_t response; } fci_device_runtime_motor_set_zero_decode_detail_t;
typedef union { motor_store_parameters_response_t response; } fci_device_runtime_motor_store_parameters_decode_detail_t;
typedef union { reboot_response_t response; } fci_device_runtime_reboot_decode_detail_t;
typedef union { release_control_lease_response_t response; } fci_device_runtime_release_control_lease_decode_detail_t;
typedef union { set_arm_control_mode_response_t response; } fci_device_runtime_set_arm_control_mode_decode_detail_t;
typedef union { set_arm_mode_response_t response; } fci_device_runtime_set_arm_mode_decode_detail_t;
typedef union { set_device_info_response_t response; } fci_device_runtime_set_device_info_decode_detail_t;
typedef union { set_device_settings_response_t response; } fci_device_runtime_set_device_settings_decode_detail_t;
typedef union { set_gripper_control_mode_response_t response; } fci_device_runtime_set_gripper_control_mode_decode_detail_t;
typedef union { set_zero_response_t response; } fci_device_runtime_set_zero_decode_detail_t;
typedef union { start_upgrade_response_t response; } fci_device_runtime_start_upgrade_decode_detail_t;
typedef struct {
  size_t storage_size;
  size_t storage_alignment;
} fci_device_runtime_requirements_t;

typedef struct {
  void *data;
  size_t size;
} fci_device_runtime_storage_t;
...
typedef struct {
  ...
  union {
    bulk_status_t bulk_status_direct_scratch;
  };
  wl_rpc_client_t rpc_client;
  fci_device_runtime_rpc_encode_scratch_t rpc_encode_scratch;
  ...
  union {
    fci_device_runtime_acquire_control_lease_decode_detail_t acquire_control_lease_scratch;
    fci_device_runtime_clear_error_decode_detail_t clear_error_scratch;
    fci_device_runtime_clear_faults_decode_detail_t clear_faults_scratch;
    fci_device_runtime_emergency_stop_decode_detail_t emergency_stop_scratch;
    fci_device_runtime_get_boot_status_decode_detail_t get_boot_status_scratch;
    fci_device_runtime_get_device_info_decode_detail_t get_device_info_scratch;
    fci_device_runtime_get_device_settings_decode_detail_t get_device_settings_scratch;
    fci_device_runtime_get_motor_feedback_decode_detail_t get_motor_feedback_scratch;
    fci_device_runtime_home_decode_detail_t home_scratch;
    fci_device_runtime_motor_register_read_decode_detail_t motor_register_read_scratch;
    fci_device_runtime_motor_register_write_decode_detail_t motor_register_write_scratch;
    fci_device_runtime_motor_set_zero_decode_detail_t motor_set_zero_scratch;
    fci_device_runtime_motor_store_parameters_decode_detail_t motor_store_parameters_scratch;
    fci_device_runtime_reboot_decode_detail_t reboot_scratch;
    fci_device_runtime_release_control_lease_decode_detail_t release_control_lease_scratch;
    fci_device_runtime_set_arm_control_mode_decode_detail_t set_arm_control_mode_scratch;
    fci_device_runtime_set_arm_mode_decode_detail_t set_arm_mode_scratch;
    fci_device_runtime_set_device_info_decode_detail_t set_device_info_scratch;
    fci_device_runtime_set_device_settings_decode_detail_t set_device_settings_scratch;
    fci_device_runtime_set_gripper_control_mode_decode_detail_t set_gripper_control_mode_scratch;
    fci_device_runtime_set_zero_decode_detail_t set_zero_scratch;
    fci_device_runtime_start_upgrade_decode_detail_t start_upgrade_scratch;
  };
} fci_device_runtime_instance_t;

typedef int32_t fci_device_runtime_init_issue_t;
enum {
  FCI_DEVICE_RUNTIME_INIT_OK = 0,
  FCI_DEVICE_RUNTIME_INIT_NULL_ARGUMENT,
  FCI_DEVICE_RUNTIME_INIT_ROLE_ENABLE,
  FCI_DEVICE_RUNTIME_INIT_RETAINED_CAPACITY,
  FCI_DEVICE_RUNTIME_INIT_RPC_CLIENT_CAPACITY,
  FCI_DEVICE_RUNTIME_INIT_RPC_SERVER_CAPACITY,
  FCI_DEVICE_RUNTIME_INIT_RPC_TIMEOUT,
  FCI_DEVICE_RUNTIME_INIT_RPC_CACHE_POLICY,
  FCI_DEVICE_RUNTIME_INIT_LAYOUT_OVERFLOW,
  FCI_DEVICE_RUNTIME_INIT_STORAGE_TOO_SMALL,
  FCI_DEVICE_RUNTIME_INIT_STORAGE_NULL,
  FCI_DEVICE_RUNTIME_INIT_STORAGE_ALIGNMENT,
  FCI_DEVICE_RUNTIME_INIT_STORAGE_OVERLAP,
  FCI_DEVICE_RUNTIME_INIT_COMPONENT
};

typedef struct {
  fci_device_runtime_init_issue_t issue;
  const char *field;
  size_t required;
  size_t provided;
} fci_device_runtime_init_diagnostic_t;
...
fci_device_runtime_result_t fci_device_acquire_control_lease_server_complete(fci_device_runtime_t *runtime,
    const fci_device_acquire_control_lease_request_token_t *token, const acquire_control_lease_response_t *response,
    wl_time_ms_t now_ms);
...
fci_device_runtime_result_t fci_device_clear_error_server_complete(fci_device_runtime_t *runtime,
    const fci_device_clear_error_request_token_t *token, const clear_error_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_clear_faults_server_complete(fci_device_runtime_t *runtime,
    const fci_device_clear_faults_request_token_t *token, const clear_faults_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_emergency_stop_server_complete(fci_device_runtime_t *runtime,
    const fci_device_emergency_stop_request_token_t *token, const emergency_stop_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_get_device_info_server_complete(fci_device_runtime_t *runtime,
    const fci_device_get_device_info_request_token_t *token, const get_device_info_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_get_device_settings_server_complete(fci_device_runtime_t *runtime,
    const fci_device_get_device_settings_request_token_t *token, const get_device_settings_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_get_motor_feedback_server_complete(fci_device_runtime_t *runtime,
    const fci_device_get_motor_feedback_request_token_t *token, const get_motor_feedback_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_home_server_complete(fci_device_runtime_t *runtime,
    const fci_device_home_request_token_t *token, const home_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_motor_register_read_server_complete(fci_device_runtime_t *runtime,
    const fci_device_motor_register_read_request_token_t *token, const motor_register_read_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_motor_register_write_server_complete(fci_device_runtime_t *runtime,
    const fci_device_motor_register_write_request_token_t *token, const motor_register_write_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_motor_set_zero_server_complete(fci_device_runtime_t *runtime,
    const fci_device_motor_set_zero_request_token_t *token, const motor_set_zero_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_motor_store_parameters_server_complete(fci_device_runtime_t *runtime,
    const fci_device_motor_store_parameters_request_token_t *token, const motor_store_parameters_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_release_control_lease_server_complete(fci_device_runtime_t *runtime,
    const fci_device_release_control_lease_request_token_t *token, const release_control_lease_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_set_arm_control_mode_server_complete(fci_device_runtime_t *runtime,
    const fci_device_set_arm_control_mode_request_token_t *token, const set_arm_control_mode_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_set_arm_mode_server_complete(fci_device_runtime_t *runtime,
    const fci_device_set_arm_mode_request_token_t *token, const set_arm_mode_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_set_device_info_server_complete(fci_device_runtime_t *runtime,
    const fci_device_set_device_info_request_token_t *token, const set_device_info_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_set_device_settings_server_complete(fci_device_runtime_t *runtime,
    const fci_device_set_device_settings_request_token_t *token, const set_device_settings_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_set_gripper_control_mode_server_complete(fci_device_runtime_t *runtime,
    const fci_device_set_gripper_control_mode_request_token_t *token, const set_gripper_control_mode_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
fci_device_runtime_result_t fci_device_set_zero_server_complete(fci_device_runtime_t *runtime,
    const fci_device_set_zero_request_token_t *token, const set_zero_response_t *response,
    wl_time_ms_t now_ms);
/* A nonzero business status is sent without a response body. */
...
#define FCI_DEVICE_HAS_DEFAULT_ENDPOINT 1
#define FCI_DEVICE_ENDPOINT_MAX_PAYLOAD 2048U
/* Layout is fixed by the local profile; CRC32C bounds also cover smaller CRCs. */
#define FCI_DEVICE_ENDPOINT_RAW_CAPACITY (FCI_DEVICE_ENDPOINT_MAX_PAYLOAD + WL_FRAME_HEADER_SIZE + WL_FRAME_MAX_CRC)
#define FCI_DEVICE_ENDPOINT_UNIT_CAPACITY (FCI_DEVICE_ENDPOINT_RAW_CAPACITY + FCI_DEVICE_ENDPOINT_RAW_CAPACITY / 254U + 2U)
#define FCI_DEVICE_ENDPOINT_CONTROL_CAPACITY (WL_FRAME_HEADER_SIZE + WL_FRAME_MAX_CRC + 2U)
#ifndef FCI_DEVICE_ENDPOINT_RX_FIFO_CAPACITY
#define FCI_DEVICE_ENDPOINT_RX_FIFO_CAPACITY FCI_DEVICE_ENDPOINT_UNIT_CAPACITY
#endif
/* Set consistently for every TU using this endpoint; no runtime allocation. */
#ifndef FCI_DEVICE_ENDPOINT_RPC_CAPACITY
#define FCI_DEVICE_ENDPOINT_RPC_CAPACITY 4U
#endif
#if FCI_DEVICE_ENDPOINT_RPC_CAPACITY < 1 || FCI_DEVICE_ENDPOINT_RPC_CAPACITY > 65535
#error "endpoint RPC capacity must be 1..65535"
#endif
#define FCI_DEVICE_ENDPOINT_REQUEST_CAPACITY 226U
#define FCI_DEVICE_ENDPOINT_RUNTIME_CAPACITY (FCI_DEVICE_RUNTIME_DEFAULT_STORAGE_CAPACITY + (FCI_DEVICE_ENDPOINT_RPC_CAPACITY - 1U) * (sizeof(wl_rpc_client_slot_t) + 226U))

typedef struct {
  wl_config_t link;
  wl_environment_t environment;
  /* Expert policy/storage overrides; ordinary applications use defaults. */
  fci_device_runtime_config_t advanced;

  fci_device_bulk_status_direct_fn on_bulk_status;
  void *bulk_status_user_data;
...
typedef struct {
  struct {
    ...
    fci_device_runtime_instance_t instance;
    union {
      fci_device_runtime_default_storage_alignment_t alignment;
      uint8_t bytes[FCI_DEVICE_ENDPOINT_RUNTIME_CAPACITY];
    } arena;
    fci_device_runtime_pump_t pump;
    ...
    wl_rpc_async_slot_t submissions[FCI_DEVICE_ENDPOINT_RPC_CAPACITY];
    uint8_t requests[FCI_DEVICE_ENDPOINT_RPC_CAPACITY][FCI_DEVICE_ENDPOINT_REQUEST_CAPACITY];
    ...
    uint8_t tx_payload[FCI_DEVICE_ENDPOINT_MAX_PAYLOAD];
    uint8_t tx_unit[FCI_DEVICE_ENDPOINT_UNIT_CAPACITY];
    uint8_t control_unit[FCI_DEVICE_ENDPOINT_CONTROL_CAPACITY];
    uint8_t rx_fallback[FCI_DEVICE_ENDPOINT_UNIT_CAPACITY];
    uint8_t rx_fifo[FCI_DEVICE_ENDPOINT_RX_FIFO_CAPACITY];
  } private_state;
} fci_device_endpoint_t;

#if defined(__cplusplus)
#define FCI_DEVICE_ENDPOINT_ALIGNMENT alignof(fci_device_endpoint_t)
#elif defined(_MSC_VER)
#define FCI_DEVICE_ENDPOINT_ALIGNMENT __alignof(fci_device_endpoint_t)
#else
#define FCI_DEVICE_ENDPOINT_ALIGNMENT _Alignof(fci_device_endpoint_t)
#endif
...
static inline wl_err_t fci_device_endpoint_config_defaults(
    fci_device_endpoint_config_t *config, wl_environment_t environment) {
  int result;
  if (config == NULL) return WL_ERR_INVALID_ARG;
  memset(config, 0, sizeof(*config));
  config->link.max_payload_len = FCI_DEVICE_ENDPOINT_MAX_PAYLOAD;
  ...
  result = fci_device_runtime_config_defaults(&config->advanced);
  if (result != WL_OK) return result;
  config->advanced.rpc_client_enabled = 1U;
  config->advanced.rpc_client_slot_count = FCI_DEVICE_ENDPOINT_RPC_CAPACITY;
  config->advanced.rpc_server_pending_slot_count = FCI_DEVICE_ENDPOINT_RPC_CAPACITY;
  config->advanced.rpc_server_cache_slot_count = FCI_DEVICE_ENDPOINT_RPC_CAPACITY;
...
static inline wl_endpoint_t *fci_device_endpoint_handle(fci_device_endpoint_t *endpoint) {
...
static inline fci_device_runtime_t *fci_device_endpoint_runtime(fci_device_endpoint_t *endpoint) {
  return endpoint != NULL && wl_endpoint_link(fci_device_endpoint_handle(endpoint)) != NULL
      ? &endpoint->private_state.instance.runtime : NULL;
}

static inline void fci_device_endpoint_record(void *context,
                                      const fci_device_runtime_result_t *result) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  fci_device_runtime_result_t terminal;
  if (result->domain == FCI_DEVICE_RUNTIME_NON_RX) {
    ...
    terminal = *result;
    terminal.domain = FCI_DEVICE_RUNTIME_CORE_ERROR;
    result = &terminal;
  }
  ...
  if (result->domain == FCI_DEVICE_RUNTIME_CORE_ERROR && result->detail_kind == FCI_DEVICE_RUNTIME_DETAIL_RPC &&
  ...
  if (fci_device_runtime_result_ok(&endpoint->private_state.result))
...
static inline wl_err_t fci_device_endpoint_init_config(
    fci_device_endpoint_t *endpoint, const fci_device_endpoint_config_t *config) {
  wl_storage_t link_storage;
  fci_device_runtime_storage_t storage;
  wl_pump_hooks_t hooks;
  fci_device_runtime_config_t runtime_config;
  ...
  if (wl_endpoint_link(fci_device_endpoint_handle(endpoint)) != NULL)
  ...
  if (config->on_bulk_status != NULL) {
    if (runtime_config.bulk_status_direct_handler != NULL) return WL_ERR_INVALID_ARG;
    runtime_config.bulk_status_direct_handler = config->on_bulk_status;
    runtime_config.bulk_status_direct_user_data = config->bulk_status_user_data != NULL ? config->bulk_status_user_data : config->user_data;
  }
  ...
  if (runtime_config.get_boot_status_request_handler != NULL) return WL_ERR_NOT_SUPPORTED;
  ...
  if (runtime_config.reboot_request_handler != NULL) return WL_ERR_NOT_SUPPORTED;
  ...
  if (runtime_config.start_upgrade_request_handler != NULL) return WL_ERR_NOT_SUPPORTED;
  if (endpoint->private_state.closing) return WL_ERR_REENTRANT;
  if ((runtime_config.rpc_client_enabled && runtime_config.rpc_client_slot_count > FCI_DEVICE_ENDPOINT_RPC_CAPACITY) ||
      (runtime_config.rpc_server_enabled && (runtime_config.rpc_server_pending_slot_count > FCI_DEVICE_ENDPOINT_RPC_CAPACITY ||
      runtime_config.rpc_server_cache_slot_count > FCI_DEVICE_ENDPOINT_RPC_CAPACITY))) return WL_ERR_INVALID_ARG;
  ...
  result = fci_device_runtime_init(&endpoint->private_state.instance, &runtime_config, &storage);
  if (result != WL_OK) return result;
  ...
  result = fci_device_runtime_pump_init(&endpoint->private_state.pump,
      &endpoint->private_state.instance.runtime, fci_device_endpoint_record, endpoint);
  if (result != WL_OK) return result;
  hooks = fci_device_runtime_pump_hooks(&endpoint->private_state.pump);
  ...
  if (runtime_config.rpc_client_enabled) {
    result = wl_rpc_async_init(&endpoint->private_state.async,
        ...
        FCI_DEVICE_ENDPOINT_REQUEST_CAPACITY, endpoint->private_state.incarnation);
...
static inline wl_err_t fci_device_endpoint_init(fci_device_endpoint_t *endpoint,
                                        wl_environment_t environment) {
  fci_device_endpoint_config_t config;
  int result = fci_device_endpoint_config_defaults(&config, environment);
  return result == WL_OK ? fci_device_endpoint_init_config(endpoint, &config) : result;
}
...
static inline wl_err_t fci_device_endpoint_step(fci_device_endpoint_t *endpoint) {
  int result;
  ...
  if (endpoint->private_state.pump.last_service_result != WL_RPC_OK) {
    if (fci_device_runtime_result_ok(&endpoint->private_state.result)) {
      endpoint->private_state.result.domain = FCI_DEVICE_RUNTIME_RPC_ERROR;
      endpoint->private_state.result.detail_kind = FCI_DEVICE_RUNTIME_DETAIL_RPC;
      endpoint->private_state.result.detail.rpc.rpc_result = endpoint->private_state.pump.last_service_result;
    }
    return WL_ERR_INVALID_STATE;
  }
  return fci_device_runtime_result_ok(&endpoint->private_state.result)
      ? WL_OK : WL_ERR_INVALID_STATE;
}

static inline const fci_device_runtime_result_t *fci_device_endpoint_result(const fci_device_endpoint_t *endpoint) {
  return endpoint != NULL ? &endpoint->private_state.result : NULL;
}

static inline wl_err_t fci_device_endpoint_close(fci_device_endpoint_t *endpoint) {
  if (endpoint == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.stepping || endpoint->private_state.closing) return WL_ERR_REENTRANT;
  endpoint->private_state.closing = true;
  wl_endpoint_close(fci_device_endpoint_handle(endpoint));
  {
...
static inline wl_err_t fci_device_endpoint_driver_step(void *context) {
  return fci_device_endpoint_step((fci_device_endpoint_t *)context);
}
static inline wl_err_t fci_device_endpoint_driver_close(void *context) {
  return fci_device_endpoint_close((fci_device_endpoint_t *)context);
}
/* Setup-only platform integration; ordinary code does not drive both objects. */
static inline wl_endpoint_driver_t fci_device_endpoint_driver(fci_device_endpoint_t *endpoint) {
  wl_endpoint_driver_t driver;
  driver.endpoint = fci_device_endpoint_handle(endpoint);
  driver.context = endpoint;
  driver.step = fci_device_endpoint_driver_step;
  driver.close = fci_device_endpoint_driver_close;
...
static inline wl_err_t fci_device_endpoint_create(fci_device_endpoint_t **out,
    const fci_device_endpoint_config_t *config, const wl_allocator_t *allocator) {
  fci_device_endpoint_t *endpoint;
  ...
  storage = *allocator;
  endpoint = (fci_device_endpoint_t *)storage.allocate(storage.context,
      sizeof(*endpoint), FCI_DEVICE_ENDPOINT_ALIGNMENT);
  if (endpoint == NULL) return WL_ERR_NO_MEM;
  if ((uintptr_t)endpoint % FCI_DEVICE_ENDPOINT_ALIGNMENT != 0U) {
    storage.deallocate(storage.context, endpoint, sizeof(*endpoint), FCI_DEVICE_ENDPOINT_ALIGNMENT);
    return WL_ERR_INVALID_ARG;
  }
  memset(endpoint, 0, sizeof(*endpoint));
  error = fci_device_endpoint_init_config(endpoint, config);
  if (error != WL_OK) {
    (void)fci_device_endpoint_close(endpoint);
    storage.deallocate(storage.context, endpoint, sizeof(*endpoint), FCI_DEVICE_ENDPOINT_ALIGNMENT);
    return error;
  }
...
static inline wl_err_t fci_device_endpoint_destroy(fci_device_endpoint_t **owner) {
  fci_device_endpoint_t *endpoint;
  wl_allocator_t storage;
  int error;
  ...
  error = fci_device_endpoint_close(endpoint);
  if (error != WL_OK) return error;
  *owner = NULL;
  storage.deallocate(storage.context, endpoint, sizeof(*endpoint), FCI_DEVICE_ENDPOINT_ALIGNMENT);
  return WL_OK;
...
static inline wl_err_t fci_device_endpoint_cancel(fci_device_endpoint_t *endpoint, const wl_rpc_call_t *call) {
  if (endpoint == NULL || call == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing || wl_endpoint_link(fci_device_endpoint_handle(endpoint)) == NULL) return WL_ERR_NOT_INITIALIZED;
...
static inline fci_device_send_result_t fci_device_endpoint_send_arm_status(fci_device_endpoint_t *endpoint, const arm_status_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_arm_status_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Copy an owned value and release its lease internally. NO_DATA leaves out unchanged. */
static inline wl_err_t fci_device_endpoint_read_arm_status(fci_device_endpoint_t *endpoint, arm_status_t *out) {
  fci_device_arm_status_latest_view_t view;
  fci_device_runtime_t *runtime = fci_device_endpoint_runtime(endpoint);
  int result;
  if (out == NULL) return WL_ERR_INVALID_ARG;
  if (runtime == NULL) return WL_ERR_NOT_INITIALIZED;
  result = fci_device_arm_status_latest_acquire(runtime, &view);
  if (result != WL_OK) return result;
  *out = *view.value;
  return fci_device_arm_status_latest_release(runtime, &view);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_motor_feedback(fci_device_endpoint_t *endpoint, const motor_feedback_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_motor_feedback_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Copy an owned value and release its lease internally. NO_DATA leaves out unchanged. */
static inline wl_err_t fci_device_endpoint_read_motor_feedback(fci_device_endpoint_t *endpoint, motor_feedback_t *out) {
  fci_device_motor_feedback_latest_view_t view;
  fci_device_runtime_t *runtime = fci_device_endpoint_runtime(endpoint);
  int result;
  if (out == NULL) return WL_ERR_INVALID_ARG;
  if (runtime == NULL) return WL_ERR_NOT_INITIALIZED;
  result = fci_device_motor_feedback_latest_acquire(runtime, &view);
  if (result != WL_OK) return result;
  *out = *view.value;
  return fci_device_motor_feedback_latest_release(runtime, &view);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_arm_diagnostics(fci_device_endpoint_t *endpoint, const arm_diagnostics_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_arm_diagnostics_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Copy an owned value and release its lease internally. NO_DATA leaves out unchanged. */
static inline wl_err_t fci_device_endpoint_read_arm_diagnostics(fci_device_endpoint_t *endpoint, arm_diagnostics_t *out) {
  fci_device_arm_diagnostics_latest_view_t view;
  fci_device_runtime_t *runtime = fci_device_endpoint_runtime(endpoint);
  int result;
  if (out == NULL) return WL_ERR_INVALID_ARG;
  if (runtime == NULL) return WL_ERR_NOT_INITIALIZED;
  result = fci_device_arm_diagnostics_latest_acquire(runtime, &view);
  if (result != WL_OK) return result;
  *out = *view.value;
  return fci_device_arm_diagnostics_latest_release(runtime, &view);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_joint_mit_command(fci_device_endpoint_t *endpoint, const joint_mit_command_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_joint_mit_command_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_gripper_mit_command(fci_device_endpoint_t *endpoint, const gripper_mit_command_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_gripper_mit_command_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_joint_position_velocity_command(fci_device_endpoint_t *endpoint, const joint_position_velocity_command_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_joint_position_velocity_command_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_joint_velocity_command(fci_device_endpoint_t *endpoint, const joint_velocity_command_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_joint_velocity_command_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_joint_pvt_command(fci_device_endpoint_t *endpoint, const joint_pvt_command_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_joint_pvt_command_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_cartesian_pose_command(fci_device_endpoint_t *endpoint, const cartesian_pose_command_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_cartesian_pose_command_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_cartesian_velocity_command(fci_device_endpoint_t *endpoint, const cartesian_velocity_command_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_cartesian_velocity_command_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_gripper_position_velocity_command(fci_device_endpoint_t *endpoint, const gripper_position_velocity_command_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_gripper_position_velocity_command_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_gripper_velocity_command(fci_device_endpoint_t *endpoint, const gripper_velocity_command_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_gripper_velocity_command_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_gripper_pvt_command(fci_device_endpoint_t *endpoint, const gripper_pvt_command_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_gripper_pvt_command_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_bulk_begin(fci_device_endpoint_t *endpoint, const bulk_begin_t *message) {
  wl_time_ms_t now_ms = 0U;
  (void)wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  return fci_device_bulk_begin_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_RELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_bulk_chunk(fci_device_endpoint_t *endpoint, const bulk_chunk_t *message) {
  wl_time_ms_t now_ms = 0U;
  return fci_device_bulk_chunk_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_UNRELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_bulk_end(fci_device_endpoint_t *endpoint, const bulk_end_t *message) {
  wl_time_ms_t now_ms = 0U;
  (void)wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  return fci_device_bulk_end_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_RELIABLE, now_ms);
}

/* Delivery follows this binding. Use codec sends to override explicitly. */
static inline fci_device_send_result_t fci_device_endpoint_send_bulk_abort(fci_device_endpoint_t *endpoint, const bulk_abort_t *message) {
  wl_time_ms_t now_ms = 0U;
  (void)wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  return fci_device_bulk_abort_send(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), message, WL_DELIVERY_RELIABLE, now_ms);
}

/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_acquire_control_lease_prepare(void *context,
...
static inline void fci_device_endpoint_acquire_control_lease_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_acquire_control_lease_completion_fn)callback)(user_data, &endpoint->private_state.completion,
...
static inline wl_err_t fci_device_endpoint_acquire_control_lease_submit_at(fci_device_endpoint_t *endpoint,
    const acquire_control_lease_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_acquire_control_lease_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  ...
  observer.prepare = fci_device_endpoint_acquire_control_lease_prepare;
  observer.notify = fci_device_endpoint_acquire_control_lease_notify;
  ...
  return wl_rpc_async_submit(&endpoint->private_state.async, 25097U, 25098U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_acquire_control_lease_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_acquire_control_lease_async(fci_device_endpoint_t *endpoint,
  ...
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_acquire_control_lease_submit_at(endpoint, request, timeout_ms, now_ms,
...
typedef struct {
  ...
  void *notify_context;
} fci_device_acquire_control_lease_sync_state_t;

static inline void fci_device_endpoint_acquire_control_lease_sync_done(void *context,
    const wl_rpc_completion_t *result, const acquire_control_lease_response_value_t *response) {
  fci_device_acquire_control_lease_sync_state_t *state = (fci_device_acquire_control_lease_sync_state_t *)context;
  state->result = *result;
...
static inline wl_err_t fci_device_endpoint_acquire_control_lease_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_acquire_control_lease_sync_state_t *state = (fci_device_acquire_control_lease_sync_state_t *)context;
  wl_time_ms_t now_ms;
  int error = wl_endpoint_now(fci_device_endpoint_handle(state->endpoint), &now_ms);
  if (error != WL_OK) return error;
  ...
  return fci_device_endpoint_acquire_control_lease_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_acquire_control_lease_sync_done, state, call);
}
...
static inline wl_rpc_completion_t fci_device_endpoint_acquire_control_lease_sync(fci_device_endpoint_t *endpoint,
    const acquire_control_lease_request_value_t *request, acquire_control_lease_response_value_t *response, uint32_t timeout_ms) {
  fci_device_acquire_control_lease_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_acquire_control_lease_proxy_submit};
  ...
  platform = wl_endpoint_waiter(fci_device_endpoint_handle(endpoint));
  if (platform == NULL) {
  ...
  error = fci_device_endpoint_acquire_control_lease_async(endpoint, request, timeout_ms,
      fci_device_endpoint_acquire_control_lease_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
      error = fci_device_endpoint_step(endpoint);
      if (state.done || error != WL_OK) break;
      error = wl_endpoint_get_hint(fci_device_endpoint_handle(endpoint), &hint);
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_clear_error_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_clear_error_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_clear_error_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_clear_error_submit_at(fci_device_endpoint_t *endpoint,
    const clear_error_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_clear_error_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_clear_error_prepare;
  observer.notify = fci_device_endpoint_clear_error_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 24841U, 24842U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_clear_error_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_clear_error_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_clear_error_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_clear_error_sync_state_t;

static inline void fci_device_endpoint_clear_error_sync_done(void *context,
    const wl_rpc_completion_t *result, const clear_error_response_value_t *response) {
  fci_device_clear_error_sync_state_t *state = (fci_device_clear_error_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_clear_error_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_clear_error_sync_state_t *state = (fci_device_clear_error_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_clear_error_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_clear_error_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_clear_error_sync(fci_device_endpoint_t *endpoint,
    const clear_error_request_value_t *request, clear_error_response_value_t *response, uint32_t timeout_ms) {
  fci_device_clear_error_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_clear_error_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_clear_error_async(endpoint, request, timeout_ms,
      fci_device_endpoint_clear_error_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_clear_faults_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_clear_faults_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_clear_faults_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_clear_faults_submit_at(fci_device_endpoint_t *endpoint,
    const clear_faults_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_clear_faults_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_clear_faults_prepare;
  observer.notify = fci_device_endpoint_clear_faults_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25093U, 25094U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_clear_faults_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_clear_faults_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_clear_faults_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_clear_faults_sync_state_t;

static inline void fci_device_endpoint_clear_faults_sync_done(void *context,
    const wl_rpc_completion_t *result, const clear_faults_response_value_t *response) {
  fci_device_clear_faults_sync_state_t *state = (fci_device_clear_faults_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_clear_faults_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_clear_faults_sync_state_t *state = (fci_device_clear_faults_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_clear_faults_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_clear_faults_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_clear_faults_sync(fci_device_endpoint_t *endpoint,
    const clear_faults_request_value_t *request, clear_faults_response_value_t *response, uint32_t timeout_ms) {
  fci_device_clear_faults_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_clear_faults_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_clear_faults_async(endpoint, request, timeout_ms,
      fci_device_endpoint_clear_faults_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_emergency_stop_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_emergency_stop_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_emergency_stop_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_emergency_stop_submit_at(fci_device_endpoint_t *endpoint,
    const emergency_stop_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_emergency_stop_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_emergency_stop_prepare;
  observer.notify = fci_device_endpoint_emergency_stop_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25347U, 25348U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_emergency_stop_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_emergency_stop_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_emergency_stop_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_emergency_stop_sync_state_t;

static inline void fci_device_endpoint_emergency_stop_sync_done(void *context,
    const wl_rpc_completion_t *result, const emergency_stop_response_value_t *response) {
  fci_device_emergency_stop_sync_state_t *state = (fci_device_emergency_stop_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_emergency_stop_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_emergency_stop_sync_state_t *state = (fci_device_emergency_stop_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_emergency_stop_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_emergency_stop_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_emergency_stop_sync(fci_device_endpoint_t *endpoint,
    const emergency_stop_request_value_t *request, emergency_stop_response_value_t *response, uint32_t timeout_ms) {
  fci_device_emergency_stop_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_emergency_stop_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_emergency_stop_async(endpoint, request, timeout_ms,
      fci_device_endpoint_emergency_stop_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
static inline fci_device_runtime_result_t fci_device_endpoint_get_boot_status_start(fci_device_endpoint_t *endpoint, const get_boot_status_request_t *request, uint32_t timeout_ms) {
  wl_time_ms_t now_ms = 0U;
  (void)wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  return fci_device_get_boot_status_client_start(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), fci_device_endpoint_runtime(endpoint), request, timeout_ms, now_ms);
}

static inline wl_rpc_err_t fci_device_endpoint_get_boot_status_inspect(fci_device_endpoint_t *endpoint, uint32_t operation_id, wl_rpc_client_result_t *result) {
  return fci_device_get_boot_status_client_inspect(fci_device_endpoint_runtime(endpoint), operation_id, result);
}

static inline wl_rpc_err_t fci_device_endpoint_get_boot_status_release(fci_device_endpoint_t *endpoint, uint32_t operation_id) {
  return fci_device_get_boot_status_client_release(fci_device_endpoint_runtime(endpoint), operation_id);
}

static inline fci_device_runtime_result_t fci_device_endpoint_get_boot_status_complete(fci_device_endpoint_t *endpoint, const wl_rpc_server_request_t *request, const get_boot_status_response_t *response) {
  wl_time_ms_t now_ms = 0U;
  (void)wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  return fci_device_get_boot_status_server_complete(fci_device_endpoint_runtime(endpoint), request, response, now_ms);
}

/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_get_device_info_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_get_device_info_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_get_device_info_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_get_device_info_submit_at(fci_device_endpoint_t *endpoint,
    const get_device_info_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_get_device_info_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_get_device_info_prepare;
  observer.notify = fci_device_endpoint_get_device_info_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25109U, 25110U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_get_device_info_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_get_device_info_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_get_device_info_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_get_device_info_sync_state_t;

static inline void fci_device_endpoint_get_device_info_sync_done(void *context,
    const wl_rpc_completion_t *result, const get_device_info_response_value_t *response) {
  fci_device_get_device_info_sync_state_t *state = (fci_device_get_device_info_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_get_device_info_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_get_device_info_sync_state_t *state = (fci_device_get_device_info_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_get_device_info_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_get_device_info_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_get_device_info_sync(fci_device_endpoint_t *endpoint,
    const get_device_info_request_value_t *request, get_device_info_response_value_t *response, uint32_t timeout_ms) {
  fci_device_get_device_info_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_get_device_info_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_get_device_info_async(endpoint, request, timeout_ms,
      fci_device_endpoint_get_device_info_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_get_device_settings_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_get_device_settings_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_get_device_settings_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_get_device_settings_submit_at(fci_device_endpoint_t *endpoint,
    const get_device_settings_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_get_device_settings_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_get_device_settings_prepare;
  observer.notify = fci_device_endpoint_get_device_settings_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25128U, 25129U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_get_device_settings_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_get_device_settings_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_get_device_settings_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_get_device_settings_sync_state_t;

static inline void fci_device_endpoint_get_device_settings_sync_done(void *context,
    const wl_rpc_completion_t *result, const get_device_settings_response_value_t *response) {
  fci_device_get_device_settings_sync_state_t *state = (fci_device_get_device_settings_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_get_device_settings_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_get_device_settings_sync_state_t *state = (fci_device_get_device_settings_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_get_device_settings_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_get_device_settings_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_get_device_settings_sync(fci_device_endpoint_t *endpoint,
    const get_device_settings_request_value_t *request, get_device_settings_response_value_t *response, uint32_t timeout_ms) {
  fci_device_get_device_settings_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_get_device_settings_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_get_device_settings_async(endpoint, request, timeout_ms,
      fci_device_endpoint_get_device_settings_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_get_motor_feedback_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_get_motor_feedback_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_get_motor_feedback_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_get_motor_feedback_submit_at(fci_device_endpoint_t *endpoint,
    const get_motor_feedback_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_get_motor_feedback_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_get_motor_feedback_prepare;
  observer.notify = fci_device_endpoint_get_motor_feedback_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25107U, 25108U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_get_motor_feedback_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_get_motor_feedback_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_get_motor_feedback_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_get_motor_feedback_sync_state_t;

static inline void fci_device_endpoint_get_motor_feedback_sync_done(void *context,
    const wl_rpc_completion_t *result, const get_motor_feedback_response_value_t *response) {
  fci_device_get_motor_feedback_sync_state_t *state = (fci_device_get_motor_feedback_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_get_motor_feedback_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_get_motor_feedback_sync_state_t *state = (fci_device_get_motor_feedback_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_get_motor_feedback_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_get_motor_feedback_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_get_motor_feedback_sync(fci_device_endpoint_t *endpoint,
    const get_motor_feedback_request_value_t *request, get_motor_feedback_response_value_t *response, uint32_t timeout_ms) {
  fci_device_get_motor_feedback_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_get_motor_feedback_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_get_motor_feedback_async(endpoint, request, timeout_ms,
      fci_device_endpoint_get_motor_feedback_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_home_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_home_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_home_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_home_submit_at(fci_device_endpoint_t *endpoint,
    const home_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_home_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_home_prepare;
  observer.notify = fci_device_endpoint_home_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25089U, 25090U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_home_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_home_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_home_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_home_sync_state_t;

static inline void fci_device_endpoint_home_sync_done(void *context,
    const wl_rpc_completion_t *result, const home_response_value_t *response) {
  fci_device_home_sync_state_t *state = (fci_device_home_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_home_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_home_sync_state_t *state = (fci_device_home_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_home_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_home_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_home_sync(fci_device_endpoint_t *endpoint,
    const home_request_value_t *request, home_response_value_t *response, uint32_t timeout_ms) {
  fci_device_home_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_home_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_home_async(endpoint, request, timeout_ms,
      fci_device_endpoint_home_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_motor_register_read_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_motor_register_read_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_motor_register_read_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_motor_register_read_submit_at(fci_device_endpoint_t *endpoint,
    const motor_register_read_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_motor_register_read_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_motor_register_read_prepare;
  observer.notify = fci_device_endpoint_motor_register_read_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25117U, 25118U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_motor_register_read_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_motor_register_read_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_motor_register_read_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_motor_register_read_sync_state_t;

static inline void fci_device_endpoint_motor_register_read_sync_done(void *context,
    const wl_rpc_completion_t *result, const motor_register_read_response_value_t *response) {
  fci_device_motor_register_read_sync_state_t *state = (fci_device_motor_register_read_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_motor_register_read_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_motor_register_read_sync_state_t *state = (fci_device_motor_register_read_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_motor_register_read_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_motor_register_read_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_motor_register_read_sync(fci_device_endpoint_t *endpoint,
    const motor_register_read_request_value_t *request, motor_register_read_response_value_t *response, uint32_t timeout_ms) {
  fci_device_motor_register_read_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_motor_register_read_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_motor_register_read_async(endpoint, request, timeout_ms,
      fci_device_endpoint_motor_register_read_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_motor_register_write_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_motor_register_write_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_motor_register_write_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_motor_register_write_submit_at(fci_device_endpoint_t *endpoint,
    const motor_register_write_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_motor_register_write_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_motor_register_write_prepare;
  observer.notify = fci_device_endpoint_motor_register_write_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25119U, 25120U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_motor_register_write_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_motor_register_write_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_motor_register_write_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_motor_register_write_sync_state_t;

static inline void fci_device_endpoint_motor_register_write_sync_done(void *context,
    const wl_rpc_completion_t *result, const motor_register_write_response_value_t *response) {
  fci_device_motor_register_write_sync_state_t *state = (fci_device_motor_register_write_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_motor_register_write_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_motor_register_write_sync_state_t *state = (fci_device_motor_register_write_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_motor_register_write_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_motor_register_write_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_motor_register_write_sync(fci_device_endpoint_t *endpoint,
    const motor_register_write_request_value_t *request, motor_register_write_response_value_t *response, uint32_t timeout_ms) {
  fci_device_motor_register_write_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_motor_register_write_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_motor_register_write_async(endpoint, request, timeout_ms,
      fci_device_endpoint_motor_register_write_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_motor_set_zero_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_motor_set_zero_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_motor_set_zero_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_motor_set_zero_submit_at(fci_device_endpoint_t *endpoint,
    const motor_set_zero_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_motor_set_zero_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_motor_set_zero_prepare;
  observer.notify = fci_device_endpoint_motor_set_zero_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25123U, 25124U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_motor_set_zero_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_motor_set_zero_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_motor_set_zero_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_motor_set_zero_sync_state_t;

static inline void fci_device_endpoint_motor_set_zero_sync_done(void *context,
    const wl_rpc_completion_t *result, const motor_set_zero_response_value_t *response) {
  fci_device_motor_set_zero_sync_state_t *state = (fci_device_motor_set_zero_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_motor_set_zero_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_motor_set_zero_sync_state_t *state = (fci_device_motor_set_zero_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_motor_set_zero_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_motor_set_zero_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_motor_set_zero_sync(fci_device_endpoint_t *endpoint,
    const motor_set_zero_request_value_t *request, motor_set_zero_response_value_t *response, uint32_t timeout_ms) {
  fci_device_motor_set_zero_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_motor_set_zero_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_motor_set_zero_async(endpoint, request, timeout_ms,
      fci_device_endpoint_motor_set_zero_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_motor_store_parameters_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_motor_store_parameters_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_motor_store_parameters_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_motor_store_parameters_submit_at(fci_device_endpoint_t *endpoint,
    const motor_store_parameters_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_motor_store_parameters_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_motor_store_parameters_prepare;
  observer.notify = fci_device_endpoint_motor_store_parameters_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25121U, 25122U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_motor_store_parameters_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_motor_store_parameters_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_motor_store_parameters_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_motor_store_parameters_sync_state_t;

static inline void fci_device_endpoint_motor_store_parameters_sync_done(void *context,
    const wl_rpc_completion_t *result, const motor_store_parameters_response_value_t *response) {
  fci_device_motor_store_parameters_sync_state_t *state = (fci_device_motor_store_parameters_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_motor_store_parameters_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_motor_store_parameters_sync_state_t *state = (fci_device_motor_store_parameters_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_motor_store_parameters_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_motor_store_parameters_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_motor_store_parameters_sync(fci_device_endpoint_t *endpoint,
  ...
  fci_device_motor_store_parameters_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_motor_store_parameters_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_motor_store_parameters_async(endpoint, request, timeout_ms,
      fci_device_endpoint_motor_store_parameters_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
static inline fci_device_runtime_result_t fci_device_endpoint_reboot_start(fci_device_endpoint_t *endpoint, const reboot_request_t *request, uint32_t timeout_ms) {
  wl_time_ms_t now_ms = 0U;
  (void)wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  return fci_device_reboot_client_start(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), fci_device_endpoint_runtime(endpoint), request, timeout_ms, now_ms);
}

static inline wl_rpc_err_t fci_device_endpoint_reboot_inspect(fci_device_endpoint_t *endpoint, uint32_t operation_id, wl_rpc_client_result_t *result) {
  return fci_device_reboot_client_inspect(fci_device_endpoint_runtime(endpoint), operation_id, result);
}

static inline wl_rpc_err_t fci_device_endpoint_reboot_release(fci_device_endpoint_t *endpoint, uint32_t operation_id) {
  return fci_device_reboot_client_release(fci_device_endpoint_runtime(endpoint), operation_id);
}

static inline fci_device_runtime_result_t fci_device_endpoint_reboot_complete(fci_device_endpoint_t *endpoint, const wl_rpc_server_request_t *request, const reboot_response_t *response) {
  wl_time_ms_t now_ms = 0U;
  (void)wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  return fci_device_reboot_server_complete(fci_device_endpoint_runtime(endpoint), request, response, now_ms);
}

/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_release_control_lease_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_release_control_lease_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_release_control_lease_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_release_control_lease_submit_at(fci_device_endpoint_t *endpoint,
    const release_control_lease_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_release_control_lease_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_release_control_lease_prepare;
  observer.notify = fci_device_endpoint_release_control_lease_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25099U, 25100U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_release_control_lease_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_release_control_lease_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_release_control_lease_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_release_control_lease_sync_state_t;

static inline void fci_device_endpoint_release_control_lease_sync_done(void *context,
    const wl_rpc_completion_t *result, const release_control_lease_response_value_t *response) {
  fci_device_release_control_lease_sync_state_t *state = (fci_device_release_control_lease_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_release_control_lease_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_release_control_lease_sync_state_t *state = (fci_device_release_control_lease_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_release_control_lease_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_release_control_lease_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_release_control_lease_sync(fci_device_endpoint_t *endpoint,
    const release_control_lease_request_value_t *request, release_control_lease_response_value_t *response, uint32_t timeout_ms) {
  fci_device_release_control_lease_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_release_control_lease_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_release_control_lease_async(endpoint, request, timeout_ms,
      fci_device_endpoint_release_control_lease_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_set_arm_control_mode_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_set_arm_control_mode_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_set_arm_control_mode_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_set_arm_control_mode_submit_at(fci_device_endpoint_t *endpoint,
    const set_arm_control_mode_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_set_arm_control_mode_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_set_arm_control_mode_prepare;
  observer.notify = fci_device_endpoint_set_arm_control_mode_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25113U, 25114U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_set_arm_control_mode_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_set_arm_control_mode_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_set_arm_control_mode_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_set_arm_control_mode_sync_state_t;

static inline void fci_device_endpoint_set_arm_control_mode_sync_done(void *context,
    const wl_rpc_completion_t *result, const set_arm_control_mode_response_value_t *response) {
  fci_device_set_arm_control_mode_sync_state_t *state = (fci_device_set_arm_control_mode_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_set_arm_control_mode_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_set_arm_control_mode_sync_state_t *state = (fci_device_set_arm_control_mode_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_set_arm_control_mode_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_set_arm_control_mode_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_set_arm_control_mode_sync(fci_device_endpoint_t *endpoint,
    const set_arm_control_mode_request_value_t *request, set_arm_control_mode_response_value_t *response, uint32_t timeout_ms) {
  fci_device_set_arm_control_mode_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_set_arm_control_mode_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_set_arm_control_mode_async(endpoint, request, timeout_ms,
      fci_device_endpoint_set_arm_control_mode_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_set_arm_mode_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_set_arm_mode_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_set_arm_mode_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_set_arm_mode_submit_at(fci_device_endpoint_t *endpoint,
    const set_arm_mode_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_set_arm_mode_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_set_arm_mode_prepare;
  observer.notify = fci_device_endpoint_set_arm_mode_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25125U, 25126U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_set_arm_mode_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_set_arm_mode_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_set_arm_mode_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_set_arm_mode_sync_state_t;

static inline void fci_device_endpoint_set_arm_mode_sync_done(void *context,
    const wl_rpc_completion_t *result, const set_arm_mode_response_value_t *response) {
  fci_device_set_arm_mode_sync_state_t *state = (fci_device_set_arm_mode_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_set_arm_mode_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_set_arm_mode_sync_state_t *state = (fci_device_set_arm_mode_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_set_arm_mode_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_set_arm_mode_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_set_arm_mode_sync(fci_device_endpoint_t *endpoint,
    const set_arm_mode_request_value_t *request, set_arm_mode_response_value_t *response, uint32_t timeout_ms) {
  fci_device_set_arm_mode_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_set_arm_mode_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_set_arm_mode_async(endpoint, request, timeout_ms,
      fci_device_endpoint_set_arm_mode_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_set_device_info_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_set_device_info_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_set_device_info_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_set_device_info_submit_at(fci_device_endpoint_t *endpoint,
    const set_device_info_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_set_device_info_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_set_device_info_prepare;
  observer.notify = fci_device_endpoint_set_device_info_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25111U, 25112U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_set_device_info_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_set_device_info_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_set_device_info_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_set_device_info_sync_state_t;

static inline void fci_device_endpoint_set_device_info_sync_done(void *context,
    const wl_rpc_completion_t *result, const set_device_info_response_value_t *response) {
  fci_device_set_device_info_sync_state_t *state = (fci_device_set_device_info_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_set_device_info_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_set_device_info_sync_state_t *state = (fci_device_set_device_info_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_set_device_info_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_set_device_info_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_set_device_info_sync(fci_device_endpoint_t *endpoint,
    const set_device_info_request_value_t *request, set_device_info_response_value_t *response, uint32_t timeout_ms) {
  fci_device_set_device_info_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_set_device_info_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_set_device_info_async(endpoint, request, timeout_ms,
      fci_device_endpoint_set_device_info_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_set_device_settings_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_set_device_settings_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_set_device_settings_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_set_device_settings_submit_at(fci_device_endpoint_t *endpoint,
    const set_device_settings_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_set_device_settings_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_set_device_settings_prepare;
  observer.notify = fci_device_endpoint_set_device_settings_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25130U, 25131U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_set_device_settings_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_set_device_settings_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_set_device_settings_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_set_device_settings_sync_state_t;

static inline void fci_device_endpoint_set_device_settings_sync_done(void *context,
    const wl_rpc_completion_t *result, const set_device_settings_response_value_t *response) {
  fci_device_set_device_settings_sync_state_t *state = (fci_device_set_device_settings_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_set_device_settings_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_set_device_settings_sync_state_t *state = (fci_device_set_device_settings_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_set_device_settings_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_set_device_settings_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_set_device_settings_sync(fci_device_endpoint_t *endpoint,
    const set_device_settings_request_value_t *request, set_device_settings_response_value_t *response, uint32_t timeout_ms) {
  fci_device_set_device_settings_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_set_device_settings_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_set_device_settings_async(endpoint, request, timeout_ms,
      fci_device_endpoint_set_device_settings_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_set_gripper_control_mode_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_set_gripper_control_mode_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_set_gripper_control_mode_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_set_gripper_control_mode_submit_at(fci_device_endpoint_t *endpoint,
    const set_gripper_control_mode_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_set_gripper_control_mode_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_set_gripper_control_mode_prepare;
  observer.notify = fci_device_endpoint_set_gripper_control_mode_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 25115U, 25116U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_set_gripper_control_mode_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_set_gripper_control_mode_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_set_gripper_control_mode_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_set_gripper_control_mode_sync_state_t;

static inline void fci_device_endpoint_set_gripper_control_mode_sync_done(void *context,
    const wl_rpc_completion_t *result, const set_gripper_control_mode_response_value_t *response) {
  fci_device_set_gripper_control_mode_sync_state_t *state = (fci_device_set_gripper_control_mode_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_set_gripper_control_mode_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_set_gripper_control_mode_sync_state_t *state = (fci_device_set_gripper_control_mode_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_set_gripper_control_mode_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_set_gripper_control_mode_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_set_gripper_control_mode_sync(fci_device_endpoint_t *endpoint,
  ...
  fci_device_set_gripper_control_mode_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_set_gripper_control_mode_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_set_gripper_control_mode_async(endpoint, request, timeout_ms,
      fci_device_endpoint_set_gripper_control_mode_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
/* Generator internals: all services reuse one bounded completion scratch. */
static inline void fci_device_endpoint_set_zero_prepare(void *context,
    const wl_rpc_client_result_t *client) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  wl_rpc_completion_t *result = &endpoint->private_state.completion;
  wl_rpc_async_completion(client, result);
...
static inline void fci_device_endpoint_set_zero_notify(void *context,
    wl_rpc_callback_t callback, void *user_data) {
  fci_device_endpoint_t *endpoint = (fci_device_endpoint_t *)context;
  ((fci_device_set_zero_completion_fn)callback)(user_data, &endpoint->private_state.completion,
      endpoint->private_state.completion.status == WL_RPC_SUCCESS
 ...
 * fields; copy *response during the callback to save it. Never release a slot.
 * No callback on failed admission; BUSY means local bounded capacity is full. */
static inline wl_err_t fci_device_endpoint_set_zero_submit_at(fci_device_endpoint_t *endpoint,
    const set_zero_request_value_t *request, uint32_t timeout_ms, wl_time_ms_t now_ms,
    fci_device_set_zero_completion_fn callback, void *user_data, wl_rpc_call_t *out_call) {
  wl_rpc_async_observer_t observer;
  if (endpoint == NULL || request == NULL || callback == NULL) return WL_ERR_INVALID_ARG;
  if (endpoint->private_state.closing) return WL_ERR_NOT_INITIALIZED;
  observer.prepare = fci_device_endpoint_set_zero_prepare;
  observer.notify = fci_device_endpoint_set_zero_notify;
  observer.context = endpoint;
  observer.callback = (wl_rpc_callback_t)callback;
  observer.user_data = user_data;
  return wl_rpc_async_submit(&endpoint->private_state.async, 24839U, 24840U,
      WL_DELIVERY_RELIABLE, timeout_ms, now_ms, fci_device_set_zero_encode_submission, request,
      &observer, out_call);
}

static inline wl_err_t fci_device_endpoint_set_zero_async(fci_device_endpoint_t *endpoint,
  ...
  error = wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  if (error != WL_OK) return error;
  return fci_device_endpoint_set_zero_submit_at(endpoint, request, timeout_ms, now_ms,
      callback, user_data, out_call);
}
...
typedef struct {
  ...
  wl_rpc_sync_notify_fn notify;
  void *notify_context;
} fci_device_set_zero_sync_state_t;

static inline void fci_device_endpoint_set_zero_sync_done(void *context,
    const wl_rpc_completion_t *result, const set_zero_response_value_t *response) {
  fci_device_set_zero_sync_state_t *state = (fci_device_set_zero_sync_state_t *)context;
  state->result = *result;
  if (response != NULL) *state->response = *response;
  state->done = true;
  if (state->notify != NULL) state->notify(state->notify_context, &state->result);
...
static inline wl_err_t fci_device_endpoint_set_zero_proxy_submit(void *context,
    wl_time_ms_t deadline, wl_rpc_sync_notify_fn notify, void *notify_context,
    wl_rpc_call_t *call) {
  fci_device_set_zero_sync_state_t *state = (fci_device_set_zero_sync_state_t *)context;
  wl_time_ms_t now_ms;
  ...
  state->notify = notify;
  state->notify_context = notify_context;
  return fci_device_endpoint_set_zero_submit_at(state->endpoint, state->request, remaining, now_ms,
      fci_device_endpoint_set_zero_sync_done, state, call);
}
 ...
 * and platform failures use local_error, not business rejection/transport_error.
 * On return there is no remaining callback referring to this function's stack. */
static inline wl_rpc_completion_t fci_device_endpoint_set_zero_sync(fci_device_endpoint_t *endpoint,
    const set_zero_request_value_t *request, set_zero_response_value_t *response, uint32_t timeout_ms) {
  fci_device_set_zero_sync_state_t state;
  wl_rpc_call_t call;
  wl_waiter_t waiter;
  const wl_waiter_t *platform;
  const wl_rpc_executor_t *executor;
  ...
  executor = wl_endpoint_rpc_executor(fci_device_endpoint_handle(endpoint));
  if (executor != NULL) {
    const wl_rpc_sync_call_t proxy = {&state, fci_device_endpoint_set_zero_proxy_submit};
    return executor->invoke(executor->context, &proxy, timeout_ms);
  }
  if (endpoint->private_state.stepping || endpoint->private_state.closing ||
      endpoint->private_state.sync_waiting) {
  ...
  waiter = *platform;
  endpoint->private_state.sync_waiting = true;
  error = fci_device_endpoint_set_zero_async(endpoint, request, timeout_ms,
      fci_device_endpoint_set_zero_sync_done, &state, &call);
  if (error == WL_OK) {
    while (!state.done) {
      wl_poll_hint_t hint;
...
static inline fci_device_runtime_result_t fci_device_endpoint_start_upgrade_start(fci_device_endpoint_t *endpoint, const start_upgrade_request_t *request, uint32_t timeout_ms) {
  wl_time_ms_t now_ms = 0U;
  (void)wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  return fci_device_start_upgrade_client_start(wl_endpoint_link(fci_device_endpoint_handle(endpoint)), fci_device_endpoint_runtime(endpoint), request, timeout_ms, now_ms);
}

static inline wl_rpc_err_t fci_device_endpoint_start_upgrade_inspect(fci_device_endpoint_t *endpoint, uint32_t operation_id, wl_rpc_client_result_t *result) {
  return fci_device_start_upgrade_client_inspect(fci_device_endpoint_runtime(endpoint), operation_id, result);
}

static inline wl_rpc_err_t fci_device_endpoint_start_upgrade_release(fci_device_endpoint_t *endpoint, uint32_t operation_id) {
  return fci_device_start_upgrade_client_release(fci_device_endpoint_runtime(endpoint), operation_id);
}

static inline fci_device_runtime_result_t fci_device_endpoint_start_upgrade_complete(fci_device_endpoint_t *endpoint, const wl_rpc_server_request_t *request, const start_upgrade_response_t *response) {
  wl_time_ms_t now_ms = 0U;
  (void)wl_endpoint_now(fci_device_endpoint_handle(endpoint), &now_ms);
  return fci_device_start_upgrade_server_complete(fci_device_endpoint_runtime(endpoint), request, response, now_ms);
}
