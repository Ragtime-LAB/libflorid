...
AstrialUSBTransport& AstrialUSBTransport::operator=(AstrialUSBTransport&& s_other) noexcept {
    if (this != &s_other) {
        m_serial = std::move(s_other.m_serial);
        m_recv_callback = s_other.m_recv_callback;
        m_recv_context = s_other.m_recv_context;
...
void AstrialUSBTransport::setReceiveCallback(ReceiveFunctor s_callback, void* s_context) {
...
bool AstrialUSBTransport::isConnected() const {
